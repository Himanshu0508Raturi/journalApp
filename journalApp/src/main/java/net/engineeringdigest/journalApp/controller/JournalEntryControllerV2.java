...
public class JournalEntryControllerV2 {
    ...
    @GetMapping
    @Operation(summary = "Get all journal entries of a user",description = "Provide an ID to look up a specific user")
    public ResponseEntity<?> getAllGeneralEntryOfAUser() {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String userName = authentication.getName();
    ...
    public ResponseEntity<JournalEntry> getById(@PathVariable String id) {
        ...
        //the above line checks the id given in path variable is present in journalEntry list(present in POJO class) of the user or not.
        if (!collect.isEmpty()) {
